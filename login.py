...
class Login_System:
    def __init__(self, root):
        self.root = root
        self.root.title("Login System | Developed by Shipra Moharana")
        ...
        btn_login = Button(login_frame, text="Login In", command=self.login, font=("Arial Rounded MT Bold", 15), bg="#00B0F0", activebackground="#00B0F0", fg="white", activeforeground="white", cursor="hand2").place(x=50, y=300, width=250, height=35)

        hr = Label(login_frame, bg="lightgrey").place(x=50, y=370, width=250, height=2)
        or_ = Label(login_frame, text="OR", fg="lightgrey", font=("times new roman", 15, "bold"), bg="white").place(x=150, y=355)

        btn_forget = Button(login_frame, text="Forget Password?",command=self.forget_window, font=("times new roman", 13), bg="white", fg="#00759E", bd=0, activebackground="white", activeforeground="#00759E",cursor="hand2").place(x=110, y=390)

        #Frame 2--------------------------------------------
        register_frame=Frame(self.root,bd=2,relief=RIDGE,bg="white")
        register_frame.place(x=650,y=570,height=60)

        lbl_register=Label(register_frame,text="SUBSCRIBE | ",font=("times new roman",13),bg="white").place(x=0,y=20,relwidth=1)
        ...
        self.image_list = [self.im1, self.im2, self.im3]
        self.image_index = 0

        self.lbl_change_image = Label(self.root, bg="white")
        self.lbl_change_image.place(x=366, y=113, width=240, height=428)
        self.animate()

#Functions---------------------------------------------------------------------------------

    def animate(self):
        self.lbl_change_image.config(image=self.image_list[self.image_index])
            ...
            if self.employee_id.get() == "":
                ...
                if email is None:
                    ...
                    if chk == 'f':
                        ...
                        self.btn_reset=Button(self.forget_win,text="SUBMIT",command=self.validate_otp,bg="lightblue")
                        ...
                        self.btn_update=Button(self.forget_win,text="Update",command=self.update_password,state=DISABLED,bg="lightblue")
    ...
    def send_email(self, to_):
        try:
            ...
            self.otp = random.randint(1000, 9999)  # Generate a random 4-digit OTP

            subj = 'SCJ IMS- Reset Password OTP'
            msg = f'Dear Sir/Madam,\n\nYour OTP to reset your password is {str(self.otp)}.\n\nWith Regards,\nSCJ IMS Team'
